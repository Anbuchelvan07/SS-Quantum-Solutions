...
export const fetchBitcoinMarketData = async () => {
  ...
  try {
    console.log('🔄 Fetching fresh Bitcoin data from CoinGecko API')
    
    // Execute parallel API calls for efficiency
    const [currentData, historicalData] = await Promise.all([
      // Current market statistics
      axios.get(`${process.env.COINGECKO_API_URL || 'https://api.coingecko.com/api/v3'}/simple/price`, {
        params: {
          ids: 'bitcoin',
          vs_currencies: 'inr',
      ...
      axios.get(`${process.env.COINGECKO_API_URL || 'https://api.coingecko.com/api/v3'}/coins/bitcoin/market_chart`, {
        params: {
          vs_currency: 'inr',
          days: 30,
          interval: 'daily'
    ...
    console.log('✅ Bitcoin data fetched and cached successfully')
    return marketData
